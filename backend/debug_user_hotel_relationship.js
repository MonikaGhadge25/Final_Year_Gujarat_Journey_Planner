...
async function debugUserHotelRelationship() {
  try {
    ...
    // Method 4: All hotels for this email
    const allHotelsForEmail = await Hotel.find({
      $or: [
        { "hotel_details.email": user.email },
        { manager_email: user.email }
      ]
    });
    console.log('4. All hotels for this email:', allHotelsForEmail.length);
    
    if (allHotelsForEmail.length > 0) {
      allHotelsForEmail.forEach((hotel, index) => {
        console.log(`   Hotel ${index + 1}:`, hotel.hotel_details.hotel_name);
    ...
    if (!hotelByEmail) {
      ...
      if (hotelByManagerId) {
        ...
        console.log('\n💡 SOLUTION: The backend controller needs to also search by manager_email');
      }
    } else {
